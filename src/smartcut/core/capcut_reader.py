...
class CapCutProject:
    ...
    def get_source_video_paths(self) -> list[Path]:
        """Return unique source video file paths from this project's materials."""
        seen = set()
        paths = []
        for mat in self.get_video_materials():
            if not mat.path:
                continue
            p = Path(mat.path)
            if p in seen:
    ...
    def get_video_segment_for_source(self, source_path: Path) -> Optional[dict]:
        """Return the first video track segment using the given source file (raw dict).

        Used to align transcription timing with where the video sits on the timeline.
        Returns None if no segment references that source.
        """
        target_id = None
        for mat in self._content.get("materials", {}).get("videos", []):
            if Path(mat.get("path", "")) == source_path:
        ...
        for track in self._content.get("tracks", []):
            if track.get("type") != "video":
                continue
            for seg in track.get("segments", []):
                if seg.get("material_id") == target_id:
                    return seg
        return None

    def add_auto_subtitle_track(
        self,
        sentences: list[dict],
        style: Optional["TextStyle"] = None,
    ) -> None:
        """Add a text track whose materials mimic CapCut's auto-caption format.

        Each sentence dict must contain:
            'start' (sec, absolute timeline position)
            'end'   (sec, absolute timeline position)
            'text'  (display string)
            'words' (list of {'word','start','end'} in seconds, absolute timeline)

        The resulting materials carry `recognize_task_id` and `words.{text,start_time,end_time}`
        (start/end in ms relative to the segment's own start) so that
        `get_subtitle_segments()`, `smart_cut_project`, and `generate_short_captions`
        all read them as if CapCut produced them.
        """
        if not sentences:
            return

        style = style or TextStyle()
        self._content.setdefault("materials", {}).setdefault("texts", [])
        ...
        for sent in sentences:
            start_us = int(sent["start"] * MICROSECONDS_PER_SECOND)
            end_us = int(sent["end"] * MICROSECONDS_PER_SECOND)
            ...
            material = self._build_text_material(material_id, text, style)
            material["recognize_task_id"] = task_id
            material["words"] = {
                "text": words_text,
            ...
            segment = self._build_text_segment(material_id, start_us, duration_us, style.position_y)
        ...
        if text_track is None:
            text_track = {
                "attribute": 0, "flag": 0, "id": generate_uuid(),
